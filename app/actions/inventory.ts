...
import { inventory, InventoryInsert, InventorySchema, InventoryUpdateSchema } from "@/lib/schema"
...
export async function updateInventoryItem(prevState: unknown, formData: FormData) {
  ...
  try {
    ...
    const validatedFields = InventoryUpdateSchema.safeParse({
      item_name,
      category,
      quantity,
      unit,
    ...
    const result = await db.update(inventory).set(values).where(
      eq(inventory.id, Number(id))
    ).returning()

    if (result.length === 0) {
      return { error: "Item not found" }
    }
    revalidatePath(`/dashboard/team/${team_id}/inventory/${id}`)
